...
#define PROID			"0718pmPIQe"

#define ACCESS_KEY		"V2VRT29sWTlLdk1xaW5WazMxSEVRYzBoWHhtZVNJa3Q="
...
extern u8 soil_humi;
extern u8 AnimalType;               //加了这个之后，buf[256]不够，得改成512
//如果开启了 MicroLib 转换出很长的字符串（例如 0.000000），sprintf 就会导致栈溢出，让程序跑飞
unsigned short OneNet_FillBuf(char *buf)
{
	
	char text[48];
	if(ccs_data.eCO2 < 400) {
        ccs_data.eCO2 = 400; // 强制设为最低有效值，骗过平台校验
    }   
    ...
    memset(text, 0, sizeof(text));
	sprintf(text, "\"Light_lux\":{\"value\":%d},", Light_lux);
	strcat(buf, text);
    ...
    memset(text, 0, sizeof(text));
	sprintf(text, "\"ccs_data\":{\"value\":%d},", ccs_data.eCO2);
	strcat(buf, text);
    
    memset(text, 0, sizeof(text));
	sprintf(text, "\"AnimalType\":{\"value\":%d},", AnimalType);
	strcat(buf, text);
    ...
    memset(text, 0, sizeof(text));
	sprintf(text, "\"beep\":{\"value\":%s},", buzzer_info.Buzzer_Status ? "true" : "false");
	strcat(buf, text);
...
void OneNet_SendData(void)
{
	...
	MQTT_PACKET_STRUCTURE mqttPacket = {NULL, 0, 0, 0};												//协议包
	//len如果是u8,缓冲区最多智能容纳256，超过256就上传不上去。
	char buf[512];
...
void OneNet_RevPro(unsigned char *cmd)
{
    ...
    cJSON *raw_json, *params_json, *pump_json, *led_json, *beep_json, *fan_json;
	...
	switch(type)
	{
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
                ...
                if (!raw_json) return;      // 解析失败直接返回
                params_json = cJSON_GetObjectItem(raw_json,"params");
                if (!params_json) { cJSON_Delete(raw_json); return; }
                ...
                if(pump_json != NULL)
                {
                    if(pump_json->type == cJSON_True) {Pump_Set(PUMP_ON); pump_state = 1;}
                    else {Pump_Set(PUMP_OFF); pump_state = 0;}
                    action_flag = 1;
                }
                // 2. 解析并控制 LED 灯
                ...
                if(led_json != NULL)
                {
                    if(led_json->type == cJSON_True) {Led_Set(LED_ON); led_state = 1;}
                    else {Led_Set(LED_OFF); led_state = 0;}
                    action_flag = 1;
                }
                // 3. 解析并控制蜂鸣器
                ...
                if(beep_json != NULL)
                {
                    if(beep_json->type == cJSON_True) {Buzzer_Set(BUZZER_ON);beep_state = 1;}
                    else {Buzzer_Set(BUZZER_OFF); beep_state = 0;}
                    action_flag = 1;
                }
                // 4. 解析并控制风扇
                ...
                if(fan_json != NULL)
                {
                    if(fan_json->type == cJSON_True) {Fan_Set(FAN_ON);fan_state = 1;}
                    else {Fan_Set(FAN_OFF); fan_state = 0;}
                    action_flag = 1;
                }
                
                if(action_flag)
                {
                    // 如果收到云端控制，建议强制切回手动模式，避免自动逻辑立刻覆盖云端指令
                    if(System_Mode == 1)
                    {
                        System_Mode = 0;
                        LCD_ShowChinese(40, 140, "自动", RED, WHITE, 16, 0); // 清除高亮
                        LCD_ShowChinese_Inverse(80, 140, "手动", BLUE, WHITE, 16); // 高亮手动
                    }
                    UI_Sync_Status();
                }
                //内存释放
